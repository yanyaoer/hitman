...
介入点。当前实现先覆盖 `codex` → `chatgpt.com` 这条链路，命名和配置已按 multi-agent 方向收敛，后续
可继续接入 `pi`、`claude` 或其他本地 LLM agent。

当前能力：拦截本机 `codex` CLI 发往 `chatgpt.com` 的流量，**不改任何 CLI 配置**，
...
```bash
./hitman off     # 移除 sing-box 重定向规则 → codex 直连 chatgpt.com（用它自己的 token，无残留）
./hitman on      # 重新启用
```
...
本机自用）。可用 `HITMAN_AUDIT_BODIES=false` 关闭请求体记录。
...
| `HITMAN_ALLOW_HOSTS` | `chatgpt.com` | 允许转发的上游 Host（逗号分隔，精确或 `.后缀`）；空=不限制 |
...
| 流量没被拦截 | `process_name` 匹配在 SFM 下不生效 → 去掉规则里的 `process_name`（改为 host 级重定向），hitman 会透传非 codex 流量 |
| 重定向规则消失 | SFM 更新订阅覆盖了手改 → 重跑 `./hitman on`；`./hitman status` 可检测规则是否在位 |
| hitman 掉线 | launchd 自愈；或 `./hitman off` 回官方直连 |
| socks 2333 连不上（但 SFM 在跑） | sing-box `strict_route: true` 会阻止本机进程连它自己的 socks 入站。改用直连出口：`./hitman egress direct`（hitman 直接拨 chatgpt.com，由 TUN 捕获转发） |
...
- `direct`：hitman 直接拨 `chatgpt.com:443`，由 sing-box 的 TUN 捕获并按节点路由。**推荐在 SFM/strict_route 下用**。

> ⚠️ `direct` 出口**仅在按进程重定向（规则含 `process_name: ["codex"]`）时安全**：hitman 自身进程不匹配 codex，
> 故不会被回环重定向。若改用 **host 级重定向**（去掉 `process_name`），`direct` 会把 hitman 自己的上游请求也
